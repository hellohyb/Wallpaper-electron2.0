...
watch(
  ...
  (_new, _old) => {
    ...
    ipcRenderer.send('send-config', {
      config: configStr,
      favoriteList: JSON.parse(JSON.stringify(favoriteList.value))
    })
  },
  { deep: true }
)
...
<template>
  <div class="setting">
    <div class="set-item">
      ...
      <div class="items">
        ...
        <el-radio-group v-model="config.player.no">
          ...
          <el-radio :value="2">
            ...
            <el-tooltip class="box-item" effect="dark" content="不会包含空收藏夹" placement="top">
              <el-icon><InfoFilled /></el-icon>
            </el-tooltip>
          </el-radio>
          <el-radio :value="3">
            <span>本地目录:</span>
            <span style="margin: 0 2px; font-size: 12px">{{
              config.player.localPath === '' ? '请先选择目录' : config.player.localPath
            }}</span>
            <el-button
              type="primary"
              size="small"
              :icon="Edit"
              @click="selectLocalPlayerDir()"
            ></el-button>
          </el-radio>
        </el-radio-group>
      </div>
      ...
      <span style="font-size: 12px; color: teal"
        >当目录设置为本地目录时，请确保文件夹里面图片文件不为空。设置完成时请在菜单栏重新开启播放壁纸。</span
      >
    </div>
    <!-- <div class="set-item">
